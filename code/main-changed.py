...
def update_markov_prob(grid_db: List[List[Grid]], epsilon, max_len=36):
    ...
    ldp_server = ldp.GDPServer(epsilon / max_len, grid_map.size * 8,
                               lambda x: x)
    ldp_client = ldp.GDPClient(epsilon / max_len, grid_map.size * 8,
                               lambda x: x)
    start_server = ldp.GDPServer(epsilon / max_len, grid_map.size,
                                 lambda x: map_func.grid_index_map_func(x, grid_map))
    start_client = ldp.GDPClient(epsilon / max_len, grid_map.size,
                                 lambda x: map_func.grid_index_map_func(x, grid_map))
    end_server = ldp.GDPServer(epsilon / max_len, grid_map.size,
                               lambda x: map_func.grid_index_map_func(x, grid_map))
    end_client = ldp.GDPClient(epsilon / max_len, grid_map.size,
                               lambda x: map_func.grid_index_map_func(x, grid_map))


    for t in grid_db:
        # Here, we are getting the length of the original trajectory
        length = min(len(t), max_len) # Length is an integer
        # Start point
        start = t[0] # Here, we are getting a Grid object

        # CHANGE
        # Let us now implement a means of grabbing the actual dimensions from the Grid Object

        start_min: List[float] = [start.min_x, start.min_y]
        start_max: List[float] = [start.max_x, start.max_y]


        average_angle: float = map_func.grid_index_angle_func(start)

        # What is the map function? Map function is grid_index_map_func

        # What is a good way of finding the angle from the initial point and each grid?


        binary_vec: list[int] = start_client.privatise(start) # Perturbing start (creating virtual start?), that is, we are
                                                              # perturbing a Grid object

        # The next object we get is a perturbed version of the original trajectory location, except in
        # binary. Instead of doing this, we aren't going to be returning some binary representation of the
        # location, but rather, we are going to be adding LaPlacian noise to the original vector

        # DONE: Implement the `privatise` function in the `GDPClient` object
        laplace_vec: list[float] = start_client.privatise(start)
        # Using this laplacian vector, we are now going to aggregate this object, b

        # Then, they are aggregating this object. I don't fully understand how they are
        # actually aggregating things, but I will definitely need to work on this further.
        start_server.aggregate(binary_vec)

        # TODO: Create a GDP.aggregate() function allows for aggregating (lat, lon)
    ...
    # Phase 3: Adjusting, ie decoding the aggregated data.
    ldp_server.adjust()
    start_server.adjust()
    end_server.adjust()
...
'''
grid_trajectories = convert_raw_to_grid(db) # convert_raw_to_grid(db) ->
