...
def grid_index_map_func(g: Grid, grid_map: GridMap):
    ...
    i * number of columns + j
    """
    i, j = g.index

    # (x_index * (number of grids per row) + y_index)

    return i * len(grid_map.map[0]) + j


#  ================ ANGLE MAKER =====================  #
# Idea 1: Create angles from the grid indices
# Pros
# Cons
def grid_index_angle_func(g: Grid, grid_map: GridMap):
    """
    Map some angle to Grid.index: (i, j) => float
    params:
    g: Grid()
    grid_map: GridMap()
    """
    ...
    min_x, min_y = g.min_x, g.min_y
    max_x, max_y = g.max_x, g.max_y

    cell_center: List[float] = [ (min_x + max_x) / 2, (min_y + max_y) / 2 ]


    # My main concern here is now that I'm not actually sure on how to add the angle...
    # Should I really be trying to add the angle after creating the grid?
    # - Where is the angle really going to be coming from? ie, what is the
    # angle going to be in reference to? Imagine that we are doing all of the trajectories
    # from some initial point... Would we really be doing it in the mapping function?

    # - Won't the angle be coming from the start of the trajectory?


    center_angle = np.sin(cell_center)

    return center_angle


# ===================== BETTER ANGLE MAKER ================================== #
# Idea 2: Create angles from the actual trajectory data
# Pros
# Cons
def traj2angle(t: Tuple[float, float]) -> float:
    something: float = 1.0
    return something
